...
def main():
    ...
    with Progress(
        ...
        for horizon in active_horizons:
            ...
            metrics[f"ranking_{horizon}"] = evaluate_topk(val[f"edge_{horizon}"], edge_pred, cal_probs)
    ...
    for horizon in active_horizons:
        ...
        if dm is not None:
            table.add_row(f"dir_{horizon}", f"acc={dm['accuracy']:.3f}", f"auc={dm['auc']:.3f}")
        else:
            table.add_row(f"dir_{horizon}", "skipped", "resume")
        if rm is not None:
            table.add_row(f"ret_{horizon}", f"mae={rm['mae']:.4f}", f"corr={rm['correlation']:.3f}")
        else:
            table.add_row(f"ret_{horizon}", "skipped", "resume")
        if em is not None:
            table.add_row(f"edge_{horizon}", f"mae={em['mae']:.4f}", f"corr={em['correlation']:.3f}")
        else:
            table.add_row(f"edge_{horizon}", "skipped", "resume")

    console.print()
    console.print(table)
    console.print(f"\n[bold green]Saved live backend bundle to[/bold green] [dim]{output_dir}[/dim]")
