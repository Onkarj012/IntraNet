...
```bash
python scripts/morning_picks.py --model runs/intraday/resnls/best_model.pt --save-csv today_picks.csv
```
...
```bash
python scripts/morning_picks.py --model runs/intraday/resnls/best_model.pt --no-download
```
...
```bash
python scripts/morning_picks.py --model runs/lgbm/ --date 2026-03-12
```

```bash
PYTHONPATH=src python scripts/morning_picks.py --model runs/lgbm/ --date 2026-04-02 --max-price 1000 --top-n 15 --horizon H375
```
